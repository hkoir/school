...
class Command(BaseCommand):
    help = "Schedule the Carry Forward Leave Task for January 1st"
    ...
    def handle(self, *args, **kwargs):
        schedule_new_year, created = CrontabSchedule.objects.get_or_create(
            ...
            day_of_month="1",  # Runs on 1st day of the month
            month_of_year="1",  # Runs in January
        )
        ...
        if not PeriodicTask.objects.filter(name=task_name).exists():
            PeriodicTask.objects.create(
                ...
                name=task_name,
                task="leavemanagement.tasks.carry_forward_leave_task",  
                kwargs=json.dumps({}),  
                one_off=False, 
            )
